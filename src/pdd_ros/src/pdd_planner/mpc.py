...
pddobs = namedtuple("pddobs", "A b cone_type center vertex")
...
class MPC:
    ...
    Arguments
        ...
        *iter_num (int, default=4): The maximum number of iterations in mpc.
        ...
        pdd_obstacle (bool, default=False): If True, the obstacle list can be transported to pdd_solver directly, otherwise, it should be converted.
        ...
        max_edge_num (int, default=5): The maximum number of edges for the polygon obstacle considered in the pdd_solver.

        max_obs_num (int, default=5): The maximum number of obstacles considered in the pdd_solver.

        process_num (int, default=4): The number of processes to solve the pdd problem in parallel. Please set this value based on the CPU core.

        accelerated (bool, default=True): Controls the use of accelerated ADMM in the pdd_solver.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pdd_obstacle = pdd_obstacle
    ...
    def control(self, state, ref_speed=5, obstacle_list=[], **kwargs):
        ...
        if not self.pdd_obstacle:
            pdd_obs_list = self.convert_pdd_obstacle(
                obstacle_list, state, self.obstacle_order
            )
        else:
            pdd_obs_list = obstacle_list

        u_opt_array, info = self.pdd.iterative_solve(
            state_pre_array,
            self.cur_vel_array,
            ref_traj_list,
            gear_flag * ref_speed,
    ...
    def convert_pdd_obstacle(self, obstacle_list, state=None, obstacle_order=False):
        pdd_obs_list = []
        ...
        for obs in obstacle_list:
            if obs.cone_type == "norm2":
                ...
                pdd_obs = pddobs(A, b, obs.cone_type, obs.center, None)
                pdd_obs_list.append(pdd_obs)

            elif obs.cone_type == "Rpositive":
                A, b = self.convert_inequal_polygon(obs.vertex, obs.velocity)

                pdd_obs = pddobs(A, b, obs.cone_type, None, obs.vertex)
                pdd_obs_list.append(pdd_obs)

        if obstacle_order:
            pdd_obs_list.sort(key=self.pdd_obs_distance)

        return pdd_obs_list

    def pdd_obs_distance(self, pdd_obs):

        if pdd_obs.cone_type == "norm2":
            distance = MPC.distance(self.state[0:2], pdd_obs.center[0:2])

        elif pdd_obs.cone_type == "Rpositive":
            distance = np.min(np.linalg.norm(self.state[0:2] - pdd_obs.vertex, axis=0))
    ...
    def update_parameter(self, **kwargs):
        self.pdd.assign_adjust_parameter(**kwargs)
    ...
    def get_adjust_parameters(self):
        """
        get the adjust parameter of the pdd_solver: ws, wu, ro1, ro2, iter_threshold, slack_gain, max_sd, min_sd

        """

        return self.pdd.get_adjust_parameter()
