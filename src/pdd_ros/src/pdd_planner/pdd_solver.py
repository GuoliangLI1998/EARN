...
class pdd_solver:
    def __init__(self, receding, 
        ...
        self.rho_max = 1000
        self.ro1_pub = rospy.Publisher("/ro1", Float64, queue_size=10)
        self.ro2_pub = rospy.Publisher("/ro2", Float64, queue_size=10)
    ...
    def iterative_solve(self, nom_s, nom_u, ref_states, ref_speed, obstacle_list, **kwargs):
        ...
        for i in range(self.iter_num):
            ...
            start_time = time.time()
            opt_state_array, opt_velocity_array, resi_dual, resi_pri = self.pdd_solver(iter=i)
    ...
    def pdd_solver(self, iter):
        ...
        # if self.max_obs_num != 0:
            ...
            # Penalty dual decomposation:
            phi = self.compute_phi()

            beta1 = 1.2
            beta2 = 1.1
            if iter == self.iter_num - 1: # check convergence at the last iteration
                eta_th = 0.01
            else:
                eta_th = inf 

            if phi <= eta_th: # converge, Augmented Lagrangian
                self.update_zeta()
                resi_pri = self.update_xi()
                penalty = False
            else: # not converge, Penalty
                self.assign_adjust_parameter(ro1 = np.min([self.ro1.value * beta1, self.rho_max]),\
                                             ro2 = np.min([self.ro2.value * beta2, self.rho_max]))
                
                penalty = True
                resi_dual = 1
                resi_pri = 1

            self.ro1_pub.publish(self.ro1.value)
            self.ro2_pub.publish(self.ro2.value)
            self.phi_pub.publish(phi)

        if penalty: # brake the car for penalty-tuning if not converge
            nom_u = np.zeros((2, self.T))
    ...
    def compute_phi(self): 
        ...
        for obs_index, para_obs in enumerate(self.para_obstacle_list):
            ...
            Im_subg = (Im_array - self.para_dis.value - self.para_z_list[obs_index].value)  

            Im_subg_inf = np.linalg.norm(Im_subg, ord=inf)

            subg_list.append(Im_subg_inf)

        phi1 = np.max(subg_list)
        ...
        for obs_index, obs in enumerate(self.para_obstacle_list):
            for t in range(self.T):
                ...
                Hmt_inf = np.linalg.norm(Hmt, ord=inf)
                hm_list.append(Hmt_inf)

        phi2 = np.max(hm_list)

        phi = np.max([phi1, phi2])
    ...
    def LamMuZ_prob_solve(self):
        ...
        if self.process_num > 1:
            ...
            LamMuZ_list = pool.map(pdd_solver.solve_parallel, input_args)
